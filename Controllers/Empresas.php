<?php 
	class Empresas extends Controllers{
		...
		public function Empresas()
		{
			...
			$data['page_title'] = "EMPRESAS <small>Directorio Comercial</small>";
			...
			if($_POST){
				if(empty($_POST['txtNombre']) || empty($_POST['txtCodigo']) || empty($_POST['listCategoria']) || empty($_POST['listStatus']) )
				{
					...
					if($request_producto > 0 )
					{
						...
						$arrResponse = array('status' => false, 'msg' => '¡Atención! ya existe una empresa con el Código Ingresado.');		
					}else{
		...
		public function setImage(){
			if($_POST){
				if(empty($_POST['idproducto'])){
					...
					$idProducto = intval($_POST['idproducto']);
					$foto      = $_FILES['foto'];
					$imgNombre = 'emp_'.md5(date('d-m-Y H:m:s')).'.jpg';
