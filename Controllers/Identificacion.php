...
class Identificacion extends Controllers{
		...
		public function Identificacion()
		{
			...
			$data['page_tag'] = "Identificación";
			$data['page_title'] = "PQRSD CON IDENTIFICACIÓN ";
			$data['page_name'] = "identificacion";
			$data['page_functions_js'] = "functions_identificacion.js";
			$this->views->getView($this,"identificacion",$data);
		}

		public function ver(int $idpqr)
		{
			if(empty($_SESSION['permisosMod']['r'])){
				header("Location:".base_url().'/identificacion');
			}
			$data['page_tag'] = "PQRSD";
		...
		public function getIdentificaciones()
		{
			if($_SESSION['permisosMod']['r']){
				...
				for ($i=0; $i < count($arrData); $i++) {
					...
					if($_SESSION['permisosMod']['r']){
						$btnView .= ' <a title="Ver Detalle" href="'.base_url().'/identificacion/ver/'.$arrData[$i]['id'].'" target="_blanck" class="btn btn-info btn-sm"> <i class="far fa-eye"></i> </a> ';
					}
					...
					if( $arrData[$i]['archivo'] == 'vacio.png'){
						...
						$arrData[$i]['descarga'] = '<a style="font-size: 25pt;" href="'.base_url().'/Assets/images/pqr_i/'.$arrData[$i]['archivo'].'"><i class="fas fa-file-contract"></i></a>';
					}
				...
				//die();
			{
				...
				{	
					...
					if($request_anonimo > 0)
					{
						...
						if($sendEmail)
						{
							$arrResponse = array('status' => true, 'idpqr' => $$dataCorreo['codigo'], 'msg' => 'PQRSD con Identificación finalizada correctamente Ticket: '.$dataCorreo['codigo']);	
						}else
						{
				...
				//die();
			{
				...
				{	
					...
					if($request_anonimo > 0)
					{
							...
							if($sendEmail)
							{
								$arrResponse = array('status' => true, 'idpqr' => $dataCorreo['codigo'], 'msg' => 'PQRSD con Identificación aprobada y radicada correctamente Ticket: '.$dataCorreo['codigo']);	
							}else
							{
								$arrResponse = array('status' => true, 'idpqr' => $dataCorreo['codigo'], 'msg' => 'No es posible enviar correo por tener servidor virtual, Ticket: '.$dataCorreo['codigo']);
							}
				...
				//die();
			{
				...
				{	
					...
					if($request_anonimo > 0)
					{
						...
						if($sendEmail)
							{
								$arrResponse = array('status' => true, 'idpqr' => $idpqrsdR, 'msg' => 'PQRSD con Identificación reasignada correctamente Ticket: '.$idpqrsdR);	
							}else
							{
				...
				if(empty($_POST['listTipoid']) || empty($_POST['txtIdentificacion']) || empty($_POST['listSolicitanteid']) || empty($_POST['txtNombrea']) || empty($_POST['txtApellidoa']) || empty($_POST['listPaisId']) || empty($_POST['listDepartamentoid']) || empty($_POST['listMunicipioid']) || empty($_POST['listCorregimientoid']) || empty($_POST['txtDireccion']) || empty($_POST['listAreaId']) || empty($_POST['txtTelefono']) || empty($_POST['txtCodigo']) || empty($_POST['txtFecha']) || empty($_POST['listCanalId']) || empty($_POST['listMedioid']) || empty($_POST['txtemail']) || empty($_POST['listTipoSolicitudid']))
				{
					...
					if(intval($request_pqr) > 0 )
					{
						...
						if($option == 1){
							...
							if($sendEmail)
							{
								$arrResponse = array('status' => true, 'idpqr' => $txtCodigo, 'msg' => 'PQRSD con Identificación radicada correctamente Ticket: '.$txtCodigo);	
							}else
							{
